...
def create_embeddings(model, examps):
    ...
    # Create embeddings
    print(
        f"Embedding {len(examps)} examples (.={BATCH_SIZE} examples)", file=sys.stderr
    )
...
def create_suggested_assignment(
    ...
    if num_rev * max_papers_per_reviewer < num_pap * reviews_per_paper:
        raise ValueError(
            f"There are not enough reviewers ({num_rev}) review all the papers ({num_pap})"
    ...
    if num_rev * min_papers_per_reviewer > num_pap * reviews_per_paper:
        raise ValueError(
            f"There are too many reviewers ({num_rev}) to review all the papers ({num_pap})"
            f" given a constraint of {reviews_per_paper} reviews per paper and"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="A file containing numpy array of bids (0 = COI, 1 = no, 2 = maybe, 3 = yes)."
    ...
    parser.add_argument(
        ...
        help="A filename for where to load the cached reviewer-paper aggregate matrix",
    )
    parser.add_argument(
    ...
    rdb = calc_reviewer_db_mapping(
        reviewer_data, db, author_col=args.filter_field, author_field="authors"
    )
    ...
    if args.load_aggregate_matrix:
        ...
        assert reviewer_scores.shape[0] == len(
            submission_abs
    ...
    print(
        f"Done calculating assignment, total score: {assignment_score}", file=sys.stderr
    )
    ...
    with open(args.suggestion_file, "w") as outf:
        for i, query in enumerate(submissions):
            ...
            assigned_reviewers = assignment[i].argsort()[-args.reviews_per_paper :][
                ::-1
            ]
            ...
            ret_dict["topSimReviewers"] = [
                {
                    "name": reviewer_names[idx],
                    "ids": reviewer_data[idx][1],
    ...
    print(
        f"Done creating suggestions, written to {args.suggestion_file}", file=sys.stderr
    )
