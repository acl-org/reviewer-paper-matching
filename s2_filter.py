...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="A json file of reviewer names and IDs",
    )
    parser.add_argument(
        "--filter_field",
        type=str,
    ...
    parser.add_argument(
        "--regex",
        action="store_true",
        help="Use a parsing algorithm with regexes that may not be 100% correct",
    )
    ...
    # Slow matching with json
        ...
        for line in sys.stdin:
            ...
            if args.filter_field == "id":
                ok = any(
                    [any([(y in mapping) for y in x["ids"]]) for x in data["authors"]]
                )
            else:
