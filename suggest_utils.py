...
def calc_reviewer_db_mapping(
    reviewers, db, author_col="name", author_field="authors", warn_under=1
):
    ...
    """
    print(
        f"Calculating reviewer-paper mapping for {len(reviewers)} reviewers and {len(db)} papers",
        file=sys.stderr,
    )
    ...
    for name, num in zip(reviewers, num_papers):
        ...
        if num < warn_under:
            print(
                f"WARNING: Reviewer {name} has {num} papers in the database",
                file=sys.stderr,
            )
    return mapping
